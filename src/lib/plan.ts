...
export const FREE_LOYALTY_LIMIT = 20;
export const FREE_FIADO_MONTH_LIMIT = 15;
...
/** Fiado criado no mês (pago na hora não conta). */
export function countFiadoThisMonth(sales: Sale[]): number {
  return sales.filter(
    (s) => isSameLocalMonth(s.createdAt) && s.paidAt !== s.createdAt,
  ).length;
}
...
export async function canAddFiadoThisMonth(): Promise<boolean> {
  ...
  return countFiadoThisMonth(sales) < FREE_FIADO_MONTH_LIMIT;
}
